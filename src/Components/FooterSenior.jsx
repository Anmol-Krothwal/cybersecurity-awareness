...
// Backend base (Vite). For CRA: const API_BASE = process.env.REACT_APP_API_BASE || "http://localhost:5000";
const API_BASE = import.meta.env?.VITE_API_BASE || "http://localhost:5000";

// Local fallback tips if API not reachable
...
const newsItems = [
  {
    ...
    full:
      "Cybercriminals launched a phishing campaign impersonating Microsoft security alerts, tricking users into clicking malicious links. The fake emails warned of unusual login attempts and directed users to a convincing but fraudulent sign-in page to steal credentials.",
  ...
  },
  {
    ...
    full:
      "A coordinated ransomware attack disrupted multiple UK universities, locking researchers out of critical data. Investigations suggest attackers exploited weak VPN configurations. Emergency protocols were activated and backups restored.",
...
const Footer = () => {
  ...
  // Fetch approved tips from backend
  useEffect(() => {
    let alive = true;
    ...
    loadTips();
    const id = setInterval(loadTips, 60000); // refresh each minute
    return () => { alive = false; clearInterval(id); };
  }, []);

  // Rotate tips every 5s (same timing), with fade transition
  useEffect(() => {
  ...
  // Submit suggestion to backend; auto-generate a title from the text (keeps single-textarea UI)
  const submitSuggestion = async () => {
    const text = suggestText.trim();
    if (!text) {
      alert("Please write your tip idea.");
      return;
    }
    // First sentence or first ~60 chars as title
    const firstSentence = text.split(/[\.\n\r]/)[0].trim();
    const title = (firstSentence || "User Tip").slice(0, 80);
  ...
  return (
    <footer className="bg-gradient-to-br from-gray-800 to-gray-600 text-white pt-10 px-6 md:px-16 pb-6 rounded-none text-[17px] leading-relaxed">
      <div className="flex flex-col lg:flex-row justify-between gap-8">
        ...
        <div className="space-y-4">
          <img
            ...
            alt="Logo"
            className="w-44 mb-8 bg-white p-2 rounded-full shadow-lg hover:scale-105 transition-transform"
          />
        ...
        <div className="flex-1 space-y-4">
          ...
          <div
            className={`bg-gray-500 rounded-xl p-5 shadow-lg transition-opacity duration-300 ${
              fadeTip ? "opacity-100" : "opacity-0"
            }`}
          >
            <p className="text-xl font-bold flex items-center gap-2 text-yellow-300">
            ...
            {tipsError && <p className="text-yellow-200 mt-2 text-base">{tipsError}</p>}
          </div>
          <div className="bg-gray-500 rounded-xl p-5 shadow-lg">
            <p className="text-xl font-bold flex items-center gap-2">🧠 Cyber Slang</p>
            <p className="text-white mt-1">
          ...
          <button
            ...
            className="mt-3 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-2 text-lg transform hover:scale-[1.02] transition-transform"
          >
            💬 Suggest a Tip
          </button>
        </div>
            ...
            {newsItems.map((item) => (
              ...
              >
                <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300 z-10">
                  <span className="text-white text-base font-semibold bg-black/60 px-4 py-2 rounded-lg">
                    🔍 Click to read more
                  </span>
                </div>

                <div className="bg-gray-500 p-4 flex items-start gap-4 rounded-xl shadow-lg">
                  <img
                    src={item.image}
                    alt="News"
                    className="w-20 h-20 rounded-md object-cover transform group-hover:scale-110 transition-transform duration-300"
                  />
                  <div>
                    <p className="font-semibold text-lg">{item.title}</p>
                    <p className="text-white/80 text-base">{item.brief}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Footer Links */}
      <div className="flex flex-col md:flex-row justify-between items-center border-t border-white/30 mt-10 pt-5 text-base text-white/70">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 animate-fadeIn">
          <div className="bg-white rounded-xl p-6 max-w-md w-full text-black shadow-2xl transform animate-scaleIn">
            <h3 className="text-2xl font-bold mb-4">💡 Suggest a Tip</h3>
            <textarea
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-5 py-2 bg-gray-300 hover:bg-gray-400 rounded text-lg"
                disabled={submitting}
              >
                Cancel
              </button>
              <button
                ...
                className="px-5 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded text-lg disabled:opacity-60"
                disabled={submitting}
              >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 animate-fadeIn">
          <div className="bg-white text-black rounded-xl p-6 max-w-lg w-full relative shadow-2xl transform animate-scaleIn">
            <button
              ...
              className="absolute top-3 right-4 text-3xl text-red-600"
            >
              ✖
            </button>
            <img
            ...
            />
            <h3 className="text-2xl font-bold mb-3">{activeNews.title}</h3>
            <p className="text-base">{activeNews.full}</p>
          </div>
        </div>
      )}
