...
// Backend base (Vite). For CRA, use: const API_BASE = process.env.REACT_APP_API_BASE || "http://localhost:5000";
const API_BASE = import.meta.env?.VITE_API_BASE || "http://localhost:5000";

// Fallback tips if API isn’t reachable
...
const Footer = () => {
  ...
  const [hoveredNewsId, setHoveredNewsId] = useState(null); // kept for parity
  ...
  // Rotate tips every 5s (same timing), with fade
  useEffect(() => {
  ...
  // POST suggestion → /api/tips (approved:false)
  const submitSuggestion = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/api/tips`, {
        ...
        body: JSON.stringify({ title: suggestTitle.trim(), text: suggestText.trim() }),
      });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
  ...
  return (
    <footer className="bg-gradient-to-br from-cyan-900 to-cyan-300 text-white pt-10 px-6 md:px-16 pb-6 rounded-none">
      <div className="flex flex-col lg:flex-row justify-between gap-8">
          ...
          <div className="flex gap-4 mt-3 text-2xl">
            {[FaFacebook, FaInstagram, FaTwitter, FaYoutube].map((Icon, i) => (
              <Icon
                key={i}
                tabIndex={0}
                className="cursor-pointer hover:scale-110 hover:text-blue-200 transition-transform focus:outline-none focus:ring-2 focus:ring-blue-200 rounded-full"
              />
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 space-y-3">
          ...
          <div
            className={`bg-gray-800 rounded-xl p-4 shadow-lg transition-opacity duration-300 ${
          ...
          <button
            ...
            className="mt-3 bg-blue-900 hover:bg-blue-700 text-white px-5 py-2 rounded-full shadow-lg flex items-center gap-2 transform hover:scale-105 transition-transform"
          >
            💬 Suggest a Tip
          </button>
        </div>
          ...
          <div className="space-y-3">
            {newsItems.map((item) => (
              ...
              >
                ...
                <div className="bg-gray-800 p-3 flex items-start gap-3 rounded-xl shadow-lg">
                  <img
                    src={item.image}
                    alt="News"
